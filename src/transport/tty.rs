            ...
            if self.buf.len() >= 8 {
                {
                    ...
                    debug!("{:?}", header);
                    let size = Cursor::new(&header[4..8]).read_u32::<BigEndian>()? as usize;
...
impl<T> Stream for TtyDecoder<T>
where
    T: Stream<Item = Chunk>,
    Error: From<T::Error>,
{
...
pub fn decode<F>(stream: F) -> impl Stream<Item = (u32, Chunk), Error = Error>
where
    F: Stream<Item = Chunk, Error = Error>,
{
    TtyDecoder {
        state: State::Header,
        buf: VecDeque::new(),
